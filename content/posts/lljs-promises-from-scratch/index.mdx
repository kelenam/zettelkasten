...
- The main ways we interact with a promise is by calling `.then(fn)` and .`catch(fn)` methods.
    - Both of these end of creating new promises, which are dependent on the result of the parent promise
![](parent-promise.png)
...
- to get around this we are giong to use a `setTimeout` with no time delay.
```js {17,24}
const state = {
    PENDING: 'pending',
    FUFILLED: 'fufilled',

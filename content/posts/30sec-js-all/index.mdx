...
- I think the hickup comes remembering that the callback, and how its effectively defined is dependent on the implemented of `.every()`. `.every()` as we define it in `all` is how it uses that callback (`fn`) to be applied to each item in the array.

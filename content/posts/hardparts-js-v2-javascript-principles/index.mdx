---
title: JavaScript Principles (JSHPv2)
date: 2020-02-27
...
2. When JS code runs, it does what two things, or depends on what two things?
...
- **Line 8**: we are declaring a new constant called `newOuput` and similarly to `output` we don't immediately assign it anything, because we must create a new execution context to run `multiplyBy2` with the argument `10`. It runs, completes the above and finally gets assigned the value `20`.
...
2. Thread of Execution (going through code line by line) and Memory (a place to store data).
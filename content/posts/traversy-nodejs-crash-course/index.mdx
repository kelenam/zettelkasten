...
`console.log(path.join(__dirname), 'test', 'hello.html');`  
- the abovetakes, the dirname, a folder called test, and a file called hello.html. 
...
- The error handling isn't really we do with the 500 isn't too great, but it will do for now.
...
- So now, if we include CSS in our HTML pages, they work and everything is working nicely, if you open the network tab of our pages working you will see that the document is giving the right status codes and content types of our files.

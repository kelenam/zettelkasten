...
So where is all this code and data coming from?
...
We need to determine how to handle requests on that server(that computer).
...
So the this server (this computer) is receving messages, where? On that computers *network card* so what languages would you use to command, to make instructutions to that computer to handle the network request? Some lanuguages would be, PHP, Java, Ruby, C, C++, JS.
- Like a server, the server needs to server data back to the clients.
...
We know for sure we cannot use any of those internal features of the server. But that's the critical thing we need to do? What language would allow us to interface with the machine? Well C++
...
To do so we are going to use JS labels to leverage Node/C++ features (they look like JS functions but under the hood they are commands to Node C++ features) and that will communicate with the Network.
...
It's a language that does 3 thigns (and 1 involves a lot of help from c++)
1. Saves data and functionality (code)
2. Uses that data by running functionality (code) on it
3. Has a ton of built-in labels that triggers Node features taht are built in C++ to use our computers internals.
...
***That's all JS really does. It saves data and code and runs that code on the data***.
...
We can set up, with a JS label, a Node.js feature (ans so computer internals) to wait for requests for html/css/js tweets from our users.

How? The most powerful built-in Node feature of all: **HTTP** (and its associtaed with built-in label in JS- also `http` conventiently).

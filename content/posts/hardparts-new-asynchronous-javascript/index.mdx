...
We have a conundrum - a tension between wanting to *delay some code execution* but *not wanting to blockg the thread* from ant further code running while we wait.
...
## AsynchronousWeb Browser APIs 
So to get around this we have to introduce a whole new world of features that happen oustide of the JS engine itself, these are the features of the web browser. 

The only way to solve the 3 goals listed above is to tackle the 
